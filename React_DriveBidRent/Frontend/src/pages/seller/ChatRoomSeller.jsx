...
export default function ChatRoomSeller({ chatIdProp }) {
  ...
  useEffect(() => {
    ...
    // prevent auto-scroll when user just sent a message or when switching chats
  ...
  if (!chatId) {
    return (
      <div className="p-6 text-center text-gray-500">Select a conversation from the left to view messages.</div>
    );
  }

  return (
    <div className="flex flex-col h-full">
      <RentalChatHeader chat={chat} otherUser={chat?.buyer} carName={chat?.rentalRequest?.vehicleName} rentalPeriod={`${chat?.rentalRequest ? new Date(chat?.rentalRequest?.pickupDate).toLocaleDateString() + ' - ' + new Date(chat?.rentalRequest?.dropDate).toLocaleDateString() : ''}`} />
      <div ref={scrollContainerRef} className="flex-1 overflow-auto py-4 bg-gray-50">
        {messages.map(m => {
          ...
          console.log('Seller Message alignment - messageId:', m._id, 'senderId:', m.sender?._id, 'myUserId:', myUserId, 'isOwn:', isOwn);
          return (
