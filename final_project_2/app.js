...
const RentalRequest = require("./models/RentalRequest"); // Import RentalRequest model
...
app.get("/", async (req, res) => {
  try {
    // Fetch top 4 available rentals
    const topRentals = await RentalRequest.find({ status: 'available' })
      .sort({ createdAt: -1 }) // Sort by newest first
      .limit(4);

    // Fetch top 4 approved auctions that have started
    const topAuctions = await AuctionRequest.find({ 
      status: 'approved', 
    })
      .sort({ auctionDate: -1 }) // Sort by auction date, newest first
      .limit(4);

    res.render("homepage.ejs", { topRentals, topAuctions });
  } catch (err) {
    console.error("Error fetching homepage data:", err);
    res.render("homepage.ejs", { topRentals: [], topAuctions: [], error: "Failed to load data" });
  }
});
